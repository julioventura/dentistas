...
export class LoginComponent {
  ...
  constructor(
    ...
    private firestore: AngularFirestore  // Adicione aqui
  ) { }
    ...
    this.auth.signInWithEmailAndPassword(this.email, this.password)
      .then((userCredential) => {
        ...
        if (user) {
          ...
          // Exemplo de uso do firestore para gravar dados após login
          const userData = {
            email: user.email,
            uid: user.uid,
            loginTime: firebase.firestore.FieldValue.serverTimestamp() // Grava o horário do login
          };

          this.firestore.collection('usuarios').doc(user.uid).set(userData, { merge: true })
            .then(() => {
              console.log('Dados do usuário registrados no Firestore');
            })
            .catch(error => {
              console.error('Erro ao registrar os dados no Firestore:', error);
            });
  ...
  promptUserRegistration() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && result.confirm) {
        ...
        console.log("Apelido fornecido:", result.apelido); // Adiciona o log para verificar se o apelido foi passado
        this.createAccount(result.name, result.apelido); // Agora passamos o apelido também
      } else {
  ...
  createAccount(name: string, apelido: string) {
    ...
    this.auth.createUserWithEmailAndPassword(this.email, this.password)
      .then((userCredential) => {
        ...
        if (user && user.email) {
          ...
          // Atualiza o perfil do usuário com o nome e apelido fornecidos
          user.updateProfile({ displayName: name }).then(() => {
            console.log("Perfil do usuário atualizado com o nome:", name);

            // Salva os dados do usuário no Firestore, incluindo nome e apelido
            ...
            // Salvando na coleção correta do Firestore
            if (user.email){
              this.firestore.collection('usuarios/dentistascombr/users').doc(user.email).set(userData).then(() => {
