...
export class UserService {
  ...
  // Define o apelido no Firestore ou outro banco de dados
  setApelido(uid: string, apelido: string): void {
    const userDoc = this.firestore.collection('users').doc(uid);
    userDoc.set({ apelido }, { merge: true }) // Atualiza o campo 'apelido'
      .then(() => {
        console.log("Apelido salvo com sucesso no Firestore:", apelido);
      })
      .catch(error => {
        console.error("Erro ao salvar o apelido no Firestore:", error);
      });
  }

  saveUserProfile(userId: string, userData: any): Promise<void> {
    return this.firestore.collection('usuarios/dentistascombr/users').doc(userData.email).set(userData);
  }
